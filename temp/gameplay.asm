...
;include/player.h:59: return p->world_y - (int16_t)cam_py;
	...
	ld	c, (hl)
	sub	a, c
;src/gameplay.c:262: move_metasprite(icon1_metasprites[player.anim_frame], 0, 0, PLAYER_SCREEN_X + 8, py + 16);
	add	a, #0x10
	ld	c, a
	ldhl	sp,	#8
	ld	l, (hl)
...
;src/gameplay.c:263: move_bkg((uint8_t)(cam_px - PLAYER_SCREEN_X), (uint8_t)cam_py);
	ldhl	sp,	#20
	ld	c, (hl)
	ldhl	sp,	#38
	ld	a, (hl)
...
;src/gameplay.c:263: move_bkg((uint8_t)(cam_px - PLAYER_SCREEN_X), (uint8_t)cam_py);
	jp	00137$
00138$:
;src/gameplay.c:266: HIDE_SPRITES;
	ldh	a, (_LCDC_REG + 0)
	and	a, #0xfd
	ldh	(_LCDC_REG + 0), a
;c:\gbdk\include\gb\gb.h:1461: SCX_REG=x, SCY_REG=y;
	...
	xor	a, a
	ldh	(_SCY_REG + 0), a
;src/gameplay.c:268: waitpadup();
	call	_waitpadup
;c:\gbdk\include\gb\gb.h:811: __asm__("di");
	di
;src/gameplay.c:270: setup_menu_font();
	call	_setup_menu_font
;c:\gbdk\include\gb\gb.h:795: __asm__("ei");
	ei
;src/gameplay.c:272: redraw = 1;
	ld	hl, #_redraw
	ld	(hl), #0x01
;src/gameplay.c:273: }
	add	sp, #40
