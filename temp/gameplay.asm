...
;src/gameplay.c:158: uint16_t col = (player.world_x + 8) >> 4;
	...
	rla
	ld	b, a
;src/gameplay.c:159: if (col == 566) player.mode = MODE_SHIP;
	ld	a, h
	sub	a, #0x36
	jr	NZ, 00145$
	ld	a, b
	...
	jr	00148$
00145$:
;src/gameplay.c:160: else if (col == 707) player.mode = MODE_CUBE;
	ld	a, h
	sub	a, #0xc3
	jr	NZ, 00148$
	ld	a, b
...
00148$:
;src/gameplay.c:82: player_init(&player, 0, 240);
;src/gameplay.c:164: died = player_update(&player, joy, level_map, level_map_w, level_map_h, level_map_bank);
	ldhl	sp,	#21
	ld	a, (hl-)
	dec	hl
	push	af
	...
	ldhl	sp,	#39
	ld	(hl), a
;src/gameplay.c:167: py = player_screen_y(&player, cam_py);
	ldhl	sp,	#24
	ld	e, (hl)
	inc	hl
	ld	d, (hl)
	...
	call	___sdcc_bcall_ehl
	add	sp, #4
;src/gameplay.c:168: if (py < CAM_Y_TOP_ZONE) {
	ldhl	sp,	#40
	ld	a, c
	ld	(hl+), a
	ld	(hl), b
;src/gameplay.c:171: if ((uint16_t)target_cam_py > cam_py_max) target_cam_py = (int16_t)cam_py_max;
	ldhl	sp,	#26
	ld	a, (hl)
	ldhl	sp,	#0
	ld	(hl), a
...
00502$:
	jr	NC, 00160$
;src/gameplay.c:169: int16_t target_cam_py = player.world_y - CAM_Y_TOP_ZONE;
	ldhl	sp,	#4
	ld	a, (hl+)
	ld	c, a
	ld	b, (hl)
	...
	adc	a, #0xff
	ld	b, a
;src/gameplay.c:170: if (target_cam_py < 0) target_cam_py = 0;
	ld	h, b
	bit	7, h
	jr	Z, 00150$
	ld	bc, #0x0000
...
;src/gameplay.c:171: if ((uint16_t)target_cam_py > cam_py_max) target_cam_py = (int16_t)cam_py_max;
	...
	push	bc
00152$:
;src/gameplay.c:172: cam_py = (uint16_t)target_cam_py;
	ldhl	sp,	#24
	ld	a, c
	ld	(hl+), a
	ld	(hl), b
	jr	00161$
00160$:
;src/gameplay.c:173: } else if (py > CAM_Y_BOTTOM_ZONE) {
	ldhl	sp,	#40
	ld	a, #0x64
	sub	a, (hl)
	inc	hl
	...
	sbc	a, (hl)
	jr	NC, 00161$
;src/gameplay.c:174: int16_t target_cam_py = player.world_y - CAM_Y_BOTTOM_ZONE;
	ldhl	sp,	#4
	ld	a, (hl+)
	ld	c, a
	ld	b, (hl)
...
;src/gameplay.c:177: cam_py = (uint16_t)target_cam_py;
	...
	ld	(hl), a
00161$:
;src/gameplay.c:180: if (died) {
	ldhl	sp,	#39
	ld	a, (hl)
	or	a, a
	jp	Z, 00166$
;src/gameplay.c:181: TAC_REG = 0x00;   // Stop music timer immediately
	xor	a, a
	ldh	(_TAC_REG + 0), a
;src/gameplay.c:183: NR52_REG = 0x00; // Silence
	xor	a, a
	ldh	(_NR52_REG + 0), a
;src/gameplay.c:184: for (uint8_t i = 0; i < 4; i++) wait_vbl_done();
	ld	c, #0x00
00194$:
	ld	a, c
	sub	a, #0x04
	...
	jr	00194$
00162$:
;src/gameplay.c:185: NR52_REG = 0x80;
	ld	a, #0x80
	ldh	(_NR52_REG + 0), a
;src/gameplay.c:186: NR51_REG = 0xFF;
	ld	a, #0xff
	ldh	(_NR51_REG + 0), a
;src/gameplay.c:187: NR50_REG = 0x77;
	ld	a, #0x77
	ldh	(_NR50_REG + 0), a
;src/gameplay.c:189: if (level_songs[idx]) {
	ldhl	sp,#22
	ld	a, (hl+)
	ld	e, a
	ld	d, (hl)
	...
	or	a, c
	jr	Z, 00164$
;src/gameplay.c:190: init_music_banked(level_songs[idx], song_bank[idx], l->timer_divider);
	ldhl	sp,#35
	ld	a, (hl+)
	ld	e, a
	ld	a, (hl+)
...
;c:\gbdk\include\gb\gb.h:811: __asm__("di");
	di
;src/gameplay.c:194: cam_px = 0;
	xor	a, a
	ldhl	sp,	#42
	ld	(hl+), a
	ld	(hl), a
;src/gameplay.c:195: cam_py = 112;
	ldhl	sp,	#24
	ld	a, #0x70
	ld	(hl+), a
	xor	a, a
	ld	(hl), a
;src/gameplay.c:196: scroll_acc = 0;
	ldhl	sp,	#30
;src/gameplay.c:197: loaded_r = BKG_MT_W - 1;
	...
	xor	a, a
	ld	(hl), a
;src/gameplay.c:198: player_init(&player, 0, 240);
	ld	de, #0x00f0
	push	de
	ld	de, #0x0000
	push	de
...
;src/gameplay.c:200: fill_scroll_bg(level_map, level_map_w, level_map_h, level_map_bank);
	...
	ld	d, (hl)
	call	_fill_scroll_bg
;src/gameplay.c:202: TAC_REG = 0x04;
	ld	a, #0x04
	ldh	(_TAC_REG + 0), a
;src/gameplay.c:203: music_ready = 1;
	ld	hl, #_music_ready
	ld	(hl), #0x01
;c:\gbdk\include\gb\gb.h:795: __asm__("ei");
	ei
;src/gameplay.c:204: enable_interrupts();
00166$:
;src/gameplay.c:208: uint16_t scroll_px = (cam_px > PLAYER_SCREEN_X) ? (cam_px - PLAYER_SCREEN_X) : 0;
	ldhl	sp,	#42
	ld	a, (hl-)
	dec	hl
	ld	(hl), a
...
00199$:
	ldhl	sp,	#39
;src/gameplay.c:209: uint8_t sprite_x = (cam_px < PLAYER_SCREEN_X) ? (uint8_t)cam_px : PLAYER_SCREEN_X;
	ld	a, c
	ld	(hl+), a
	ld	a, (hl+)
	sub	a, #0x20
...
00201$:
	ldhl	sp,	#1
	ld	(hl), a
;src/gameplay.c:211: py = player_screen_y(&player, cam_py);
	ldhl	sp,	#24
	ld	e, (hl)
	inc	hl
	ld	d, (hl)
	...
	ld	(hl+), a
	ld	(hl), b
;src/gameplay.c:212: if (player.mode == MODE_SHIP) {
	ldhl	sp,	#11
	ld	c, (hl)
;src/gameplay.c:214: move_metasprite_vflip(ship_metasprites[0], 0, 0, sprite_x + 8, py + 16);
	ldhl	sp,	#40
	ld	a, (hl)
	ldhl	sp,	#1
	ld	e, (hl)
...
;src/gameplay.c:214: move_metasprite_vflip(ship_metasprites[0], 0, 0, sprite_x + 8, py + 16);
	jr	00175$
00168$:
;src/gameplay.c:216: move_metasprite(ship_metasprites[0], 0, 0, sprite_x + 8, py + 16);
	ldhl	sp,	#40
	ld	a, (hl+)
	ld	b, a
	ld	c, (hl)
...
;src/gameplay.c:216: move_metasprite(ship_metasprites[0], 0, 0, sprite_x + 8, py + 16);
	jr	00175$
00174$:
;src/gameplay.c:219: if (player.gravity_flipped) {
	ldhl	sp,	#10
	ld	a, (hl)
	or	a, a
	jr	Z, 00171$
;src/gameplay.c:220: move_metasprite_vflip(icon1_metasprites[player.anim_frame], 0, 0, sprite_x + 8, py + 16);
	ldhl	sp,	#40
	ld	a, (hl+)
	ld	b, a
	ld	c, (hl)
...
;src/gameplay.c:220: move_metasprite_vflip(icon1_metasprites[player.anim_frame], 0, 0, sprite_x + 8, py + 16);
	jr	00175$
00171$:
;src/gameplay.c:222: move_metasprite(icon1_metasprites[player.anim_frame], 0, 0, sprite_x + 8, py + 16);
	ldhl	sp,	#40
	ld	a, (hl+)
	ld	b, a
	ld	c, (hl)
...
;src/gameplay.c:222: move_metasprite(icon1_metasprites[player.anim_frame], 0, 0, sprite_x + 8, py + 16);
00175$:
;src/gameplay.c:225: move_bkg((uint8_t)scroll_px, (uint8_t)cam_py);
	ldhl	sp,	#24
	ld	c, (hl)
	ldhl	sp,	#39
	ld	a, (hl)
...
;src/gameplay.c:225: move_bkg((uint8_t)scroll_px, (uint8_t)cam_py);
	jp	00177$
00178$:
;src/gameplay.c:228: HIDE_SPRITES;
	ldh	a, (_LCDC_REG + 0)
	and	a, #0xfd
	ldh	(_LCDC_REG + 0), a
;c:\gbdk\include\gb\gb.h:1461: SCX_REG=x, SCY_REG=y;
	...
	xor	a, a
	ldh	(_SCY_REG + 0), a
;src/gameplay.c:230: waitpadup();
	call	_waitpadup
;c:\gbdk\include\gb\gb.h:811: __asm__("di");
	di
;src/gameplay.c:232: setup_menu_font();
	ld	e, #b_setup_menu_font
	ld	hl, #_setup_menu_font
	call	___sdcc_bcall_ehl
;c:\gbdk\include\gb\gb.h:795: __asm__("ei");
	ei
;src/gameplay.c:234: redraw = 1;
	ld	hl, #_redraw
	ld	(hl), #0x01
;src/gameplay.c:235: }
	add	sp, #44
