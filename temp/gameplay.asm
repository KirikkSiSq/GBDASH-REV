...
;src/gameplay.c:126: uint16_t loaded_r = BKG_MT_W - 1;
	...
	ld	a, #0x0f
	ld	(hl+), a
;src/gameplay.c:133: player_init(&player, 0, 240);
...
;src/gameplay.c:149: OBP0_REG = 0xE4;
	ld	a, #0xe4
	ldh	(_OBP0_REG + 0), a
;src/gameplay.c:150: SPRITES_8x16;
	ldh	a, (_LCDC_REG + 0)
	or	a, #0x04
	ldh	(_LCDC_REG + 0), a
...
;src/gameplay.c:157: waitpadup();
	call	_waitpadup
;src/gameplay.c:159: uint16_t scroll_acc = 0;
	ldhl	sp,	#27
;src/gameplay.c:160: uint8_t prev_joy = 0;
	xor	a, a
	ld	(hl+), a
	ld	(hl), a
;src/gameplay.c:161: while (1) {
	ld	de, #_song_bank
	ldhl	sp,	#39
	ld	l, (hl)
	ld	h, #0x00
	...
	ld	(hl), a
00140$:
;src/gameplay.c:162: wait_vbl_done();
	call	_wait_vbl_done
;src/gameplay.c:163: uint8_t joy = joypad();
	call	_joypad
	ld	c, a
;src/gameplay.c:164: if (joy & J_START) break;
	bit	7, c
	jp	NZ, 00141$
;src/gameplay.c:167: if ((joy & J_B) && !(prev_joy & J_B)) {
	bit	5, c
	jr	Z, 00109$
	push	hl
	ldhl	sp,	#30
	...
	pop	hl
	jr	NZ, 00109$
;src/gameplay.c:168: player_noclip = !player_noclip;
	...
	ld	(hl), a
00109$:
;src/gameplay.c:170: prev_joy = joy;
	ldhl	sp,	#28
	ld	(hl), c
;src/gameplay.c:173: if (cam_px < ((level_map_w - VIEW_MT_W) << 4)) {
	ldhl	sp,	#14
	ld	a, (hl+)
	ld	b, (hl)
	add	a, #0xf6
	...
	sbc	a, e
	jp	NC, 00117$
;src/gameplay.c:174: uint16_t px_prev = cam_px >> 4;
	inc	hl
	inc	hl
	ld	a, (hl)
	ldhl	sp,	#37
	...
	dec	hl
	rr	(hl)
;src/gameplay.c:175: scroll_acc += SCROLL_SPEED_FP;
	ldhl	sp,	#27
	ld	e, (hl)
	ld	d, #0x00
	ld	hl, #0x035c
	...
	ld	e, l
	ld	d, h
;src/gameplay.c:176: cam_px += scroll_acc >> 8;
	ldhl	sp,	#35
	ld	a, d
	ld	(hl+), a
	xor	a, a
	...
	ldhl	sp,	#41
	ld	(hl), a
;src/gameplay.c:177: scroll_acc &= 0xFF;
	ldhl	sp,	#27
	ld	(hl), e
;src/gameplay.c:178: uint16_t px_curr = cam_px >> 4;
	ldhl	sp,#40
	ld	a, (hl+)
	ld	e, a
	ld	d, (hl)
	...
	srl	d
	rr	e
;src/gameplay.c:179: if (px_curr != px_prev) {
	ldhl	sp,	#37
	ld	a, (hl)
	sub	a, e
	jr	NZ, 00340$
	...
	jr	Z, 00117$
00340$:
;src/gameplay.c:180: uint16_t need = px_curr + VIEW_MT_W;
	ld	hl, #0x000a
	add	hl, de
	ld	e, l
	ld	d, h
;src/gameplay.c:181: if (need > loaded_r && need < level_map_w) {
	ldhl	sp,	#25
	ld	a, (hl+)
	sub	a, e
	ld	a, (hl)
	...
	sbc	a, (hl)
	jr	NC, 00117$
;src/gameplay.c:182: loaded_r = need;
	ldhl	sp,	#25
	ld	a, e
	ld	(hl+), a
	ld	(hl), d
;src/gameplay.c:183: draw_mt_column((uint8_t)(need % BKG_MT_W), need, level_map, level_map_w, level_map_h, level_map_bank);
	ld	a, e
	and	a, #0x0f
	ld	b, a
	push	bc
	...
	pop	bc
00117$:
;src/gameplay.c:188: player.world_x = cam_px;
	ldhl	sp,	#40
	ld	a, (hl)
	ldhl	sp,	#0
	ld	(hl), a
	...
	ldhl	sp,	#1
	ld	(hl), a
;src/gameplay.c:190: _prev = _current_bank;
	ldh	a, (__current_bank + 0)
	ld	b, a
;src/gameplay.c:191: SWITCH_ROM(level_map_bank);
	ldhl	sp,	#18
	ld	a, (hl)
	ldh	(__current_bank + 0), a
	ld	a, (hl)
	ld	(#_rROMB0),a
;src/gameplay.c:192: died = player_update(&player, joy, level_map, level_map_w, level_map_h);
	push	bc
	ldhl	sp,	#18
	ld	a, (hl+)
	ld	e, a
	...
	ld	(hl), a
	pop	bc
;src/gameplay.c:193: SWITCH_ROM(_prev);
	ld	a, b
	ldh	(__current_bank + 0), a
	ld	hl, #_rROMB0
	ld	(hl), b
;src/gameplay.c:196: py = player_screen_y(&player, cam_py);
;include/player.h:62: return p->world_y - (int16_t)cam_py;
	...
	ld	a, b
	sbc	a, d
;src/gameplay.c:197: if (py < CAM_Y_TOP_ZONE) {
	ldhl	sp,	#35
	ld	(hl), c
	inc	hl
	ld	(hl), a
;src/gameplay.c:200: if ((uint16_t)target_cam_py > cam_py_max) target_cam_py = (int16_t)cam_py_max;
	ldhl	sp,	#23
	ld	a, (hl)
	ldhl	sp,	#37
	ld	(hl), a
...
00342$:
	jr	NC, 00129$
;src/gameplay.c:198: int16_t target_cam_py = player.world_y - CAM_Y_TOP_ZONE;
	ldhl	sp,	#2
	ld	a, (hl+)
	ld	c, a
	ld	b, (hl)
	...
	adc	a, #0xff
	ld	b, a
;src/gameplay.c:199: if (target_cam_py < 0) target_cam_py = 0;
	ld	h, b
	bit	7, h
	jr	Z, 00119$
	ld	bc, #0x0000
...
;src/gameplay.c:200: if ((uint16_t)target_cam_py > cam_py_max) target_cam_py = (int16_t)cam_py_max;
	...
	ld	b, (hl)
00121$:
;src/gameplay.c:201: cam_py = (uint16_t)target_cam_py;
	ldhl	sp,	#21
	ld	a, c
	ld	(hl+), a
	ld	(hl), b
	jr	00130$
00129$:
;src/gameplay.c:202: } else if (py > CAM_Y_BOTTOM_ZONE) {
	ldhl	sp,	#35
	ld	a, #0x64
	sub	a, (hl)
	inc	hl
	...
	sbc	a, (hl)
	jr	NC, 00130$
;src/gameplay.c:203: int16_t target_cam_py = player.world_y - CAM_Y_BOTTOM_ZONE;
	ldhl	sp,	#2
	ld	a, (hl)
	ldhl	sp,	#35
	ld	(hl), a
	...
	adc	a, #0xff
	ld	b, a
;src/gameplay.c:204: if (target_cam_py < 0) target_cam_py = 0;
	ld	h, b
	bit	7, h
	jr	Z, 00123$
	ld	bc, #0x0000
00123$:
;src/gameplay.c:205: if ((uint16_t)target_cam_py > cam_py_max) target_cam_py = (int16_t)cam_py_max;
	ld	e, c
	ld	d, b
	ldhl	sp,	#23
	ld	a, (hl+)
	...
	ld	b, (hl)
00125$:
;src/gameplay.c:206: cam_py = (uint16_t)target_cam_py;
	ldhl	sp,	#21
	ld	a, c
	ld	(hl+), a
	ld	(hl), b
00130$:
;src/gameplay.c:209: if (died) {
	ldhl	sp,	#34
	ld	a, (hl)
	or	a, a
	jp	Z, 00135$
;src/gameplay.c:210: music_ready = 0;
;src/gameplay.c:211: TAC_REG = 0x00;   // Stop music timer immediately
	xor	a, a
	ld	(#_music_ready), a
	ldh	(_TAC_REG + 0), a
;src/gameplay.c:213: NR52_REG = 0x00; // Silence
	xor	a, a
	ldh	(_NR52_REG + 0), a
;src/gameplay.c:214: for (uint8_t i = 0; i < 4; i++) wait_vbl_done();
	ldhl	sp,	#41
	ld	(hl), #0x00
00163$:
	ldhl	sp,	#41
	...
	jr	00163$
00131$:
;src/gameplay.c:215: NR52_REG = 0x80;
	ld	a, #0x80
	ldh	(_NR52_REG + 0), a
;src/gameplay.c:216: NR51_REG = 0xFF;
	ld	a, #0xff
	ldh	(_NR51_REG + 0), a
;src/gameplay.c:217: NR50_REG = 0x77;
	ld	a, #0x77
	ldh	(_NR50_REG + 0), a
;src/gameplay.c:219: if (level_songs[idx]) {
	ldhl	sp,#19
	ld	a, (hl+)
	ld	e, a
	ld	d, (hl)
	...
	or	a, (hl)
	jr	Z, 00133$
;src/gameplay.c:224: uint8_t pb = _current_bank;
	dec	hl
	dec	hl
	ldh	a, (__current_bank + 0)
	ld	(hl), a
;src/gameplay.c:225: SWITCH_ROM(1);
	ld	a, #0x01
	ldh	(__current_bank + 0), a
	ld	hl, #_rROMB0
	ld	(hl), #0x01
;src/gameplay.c:226: song_ptr = level_songs[idx];
;src/gameplay.c:227: s_bank = song_bank[idx];
	ldhl	sp,#29
	ld	a, (hl+)
	ld	e, a
;src/gameplay.c:228: s_divider = l->timer_divider;
	ld	a, (hl+)
	ld	d, a
	ld	a, (de)
	ld	b, a
	...
	ld	a, (de)
	ld	c, a
;src/gameplay.c:229: SWITCH_ROM(pb);
	ldhl	sp,	#38
	ld	a, (hl)
	ldh	(__current_bank + 0), a
	ld	a, (hl)
	ld	(#_rROMB0),a
;src/gameplay.c:231: current_song_bank = s_bank;
	ld	hl, #_current_song_bank
	ld	(hl), b
;c:\gbdk\include\gb\gb.h:811: __asm__("di");
	di
;src/gameplay.c:233: uint8_t mb = _current_bank;
	ldh	a, (__current_bank + 0)
	ld	e, a
;src/gameplay.c:234: SWITCH_ROM(s_bank);
	ld	a, b
	ldh	(__current_bank + 0), a
	ld	hl, #_rROMB0
	ld	(hl), b
;src/gameplay.c:235: hUGE_init(song_ptr);
	push	bc
	push	de
	ldhl	sp,	#44
	ld	a, (hl+)
	...
	pop	de
	pop	bc
;src/gameplay.c:236: SWITCH_ROM(mb);
	ld	a, e
	ldh	(__current_bank + 0), a
	ld	hl, #_rROMB0
	ld	(hl), e
;src/gameplay.c:237: TMA_REG = s_divider;
	ld	a, c
	ldh	(_TMA_REG + 0), a
;c:\gbdk\include\gb\gb.h:795: __asm__("ei");
	ei
;src/gameplay.c:238: enable_interrupts();
00133$:
;c:\gbdk\include\gb\gb.h:811: __asm__("di");
	di
;src/gameplay.c:242: cam_px = 0;
	xor	a, a
	ldhl	sp,	#40
	ld	(hl+), a
	ld	(hl), a
;src/gameplay.c:243: cam_py = 112;
	ldhl	sp,	#21
	ld	a, #0x70
	ld	(hl+), a
	xor	a, a
	ld	(hl), a
;src/gameplay.c:244: scroll_acc = 0;
	ldhl	sp,	#27
;src/gameplay.c:245: loaded_r = BKG_MT_W - 1;
	xor	a, a
	ld	(hl-), a
	dec	hl
	ld	a, #0x0f
...
;c:\gbdk\include\gb\gb.h:1461: SCX_REG=x, SCY_REG=y;
	...
	ld	a, #0x70
	ldh	(_SCY_REG + 0), a
;src/gameplay.c:248: fill_scroll_bg(level_map, level_map_w, level_map_h, level_map_bank);
	ldhl	sp,	#18
	ld	a, (hl-)
	dec	hl
	push	af
	...
	ld	d, (hl)
	call	_fill_scroll_bg
;src/gameplay.c:250: TAC_REG = 0x04;
	ld	a, #0x04
	ldh	(_TAC_REG + 0), a
...
;c:\gbdk\include\gb\gb.h:795: __asm__("ei");
	ei
;src/gameplay.c:253: waitpadup();
	call	_waitpadup
00135$:
;src/gameplay.c:256: uint16_t scroll_px = (cam_px > PLAYER_SCREEN_X) ? (cam_px - PLAYER_SCREEN_X) : 0;
	ldhl	sp,	#40
	ld	a, (hl)
	ldhl	sp,	#35
	ld	(hl), a
...
00168$:
	...
	ld	a, (hl)
	ldhl	sp,	#33
;src/gameplay.c:257: uint8_t sprite_x = (cam_px < PLAYER_SCREEN_X) ? (uint8_t)cam_px : PLAYER_SCREEN_X;
	ld	(hl+), a
	inc	hl
	ld	a, (hl+)
	sub	a, #0x20
...
;include/player.h:62: return p->world_y - (int16_t)cam_py;
	...
	ld	(hl-), a
	ld	(hl), e
;src/gameplay.c:260: if (player.gravity_flipped) {
	ldhl	sp,	#8
	ld	c, (hl)
;src/gameplay.c:261: move_metasprite_vflip(icon1_metasprites[player.anim_frame], 0, 0, sprite_x + 8, py + 16);
	ldhl	sp,	#37
	ld	a, (hl)
	ldhl	sp,	#34
	ld	e, (hl)
...
;src/gameplay.c:261: move_metasprite_vflip(icon1_metasprites[player.anim_frame], 0, 0, sprite_x + 8, py + 16);
	jr	00138$
00137$:
;src/gameplay.c:263: move_metasprite(icon1_metasprites[player.anim_frame], 0, 0, sprite_x + 8, py + 16);
	ldhl	sp,	#37
	ld	c, (hl)
	ldhl	sp,	#9
	ld	l, (hl)
...
;src/gameplay.c:263: move_metasprite(icon1_metasprites[player.anim_frame], 0, 0, sprite_x + 8, py + 16);
00138$:
;src/gameplay.c:265: move_bkg((uint8_t)scroll_px, (uint8_t)cam_py);
	ldhl	sp,	#21
	ld	c, (hl)
	ldhl	sp,	#33
	ld	a, (hl)
...
;src/gameplay.c:265: move_bkg((uint8_t)scroll_px, (uint8_t)cam_py);
	jp	00140$
00141$:
;src/gameplay.c:268: HIDE_SPRITES;
	ldh	a, (_LCDC_REG + 0)
	and	a, #0xfd
	ldh	(_LCDC_REG + 0), a
;c:\gbdk\include\gb\gb.h:1461: SCX_REG=x, SCY_REG=y;
	...
	xor	a, a
	ldh	(_SCY_REG + 0), a
;src/gameplay.c:270: waitpadup();
	call	_waitpadup
;c:\gbdk\include\gb\gb.h:811: __asm__("di");
	di
;src/gameplay.c:272: setup_menu_font();
	call	_setup_menu_font
;c:\gbdk\include\gb\gb.h:795: __asm__("ei");
	ei
;src/gameplay.c:274: redraw = 1;
	ld	hl, #_redraw
	ld	(hl), #0x01
;src/gameplay.c:275: }
	add	sp, #42
